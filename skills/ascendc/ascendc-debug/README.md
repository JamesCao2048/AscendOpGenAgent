...
修复编译通过但精度测试失败的 AscendC 算子。作为 AscendOpGenAgent 中的精度调优分支：`ascend-kernel-developer` 完成生成 → 若 `evaluate_ascendc.sh` 报 Numerical 失败 → 进入本 Skill。
...
AscendOpGenAgent/
├── agents/
│   ├── ascendc-debug-discovery.md      # 发现式 Subagent（直接从取证数据推理）
│   └── ascendc-debug.md                # 构建式 Subagent（Phase A→B→C 规范化审计）
└── skills/ascendc/ascendc-debug/
    ├── SKILL.md                           # Skill 执行手册（主流程、Sub-steps、Gate 协议）
    ├── README.md                          # 本文件：设计文档
    ├── STRUCTURE.md                       # 目录结构说明
    ├── scripts/                           # 共用脚本
...
- 下游：`utils/verification_ascendc.py`（评测）、`utils/run_ascendc_debug.sh`（批量调度 + 反作弊）

## 双 Subagent 架构

### 发现式 Subagent (`ascendc-debug-discovery`)
...
**agent 文件**（`agents/ascendc-debug{,-discovery}.md`）有"反作弊约束"硬规则章节，明确唯一可改目录为 `kernel/`。
