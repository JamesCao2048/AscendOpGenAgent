...
10. **精简 PASS 不等于最终 PASS + 全量失败 subagent 自修复**: 若 `{task_dir}/{op_name}.json.bak` 存在，Gate-V 通过后必须恢复 `.json.bak -> .json` 再跑全量验证。**若全量失败，仅修改 `{task_dir}/kernel/` 文件并重跑全量，最多补救 3 次（含首次全量）**；3 次仍失败才转 Step 6 写失败报告。"全量验证 + 补救"整段在 subagent 内部完成，不回传 parent 等待外部指令
