...
1. 把两个 Markdown 形式的 agent 定义 (`agents/ascend-kernel-developer.md` & `agents/precision-tuning-discovery.md`) 迁移到 Codex 原生 TOML 形式 (`.codex/agents/`)，让 Codex 在本仓库启动时自动注册这两个 agent。
2. 重构 `ascend-kernel-developer` 的 Phase 4：**取消"转译→退化预检查→功能验证→Conductor"的三轮内循环**；Phase 4 变成一次性的线性流程，当 4.3 功能验证失败时，4.4 直接 `spawn` 新注册的 `cann-debug-agent` subagent。
3. 把精度修复循环 (取证→分析→修复→Gate 验证) 全部内聚到 `cann-debug-agent` 内部。
4. 为 `cann-debug-agent` 设计清晰的"首次输入契约"，确保它从 **parent-spawn** 被调起时拿到的上下文与它从 **standalone 启动**（`run_precision_tuning.sh`）时等效。

## 2. 非目标（YAGNI）

- 不改 `utils/run_precision_tuning.sh` 的行为；standalone 路径继续工作。
- 不改 `skills/ascendc/precision-tuning/` 下的脚本契约（forensics/gate）。
...
│       └── cann-debug-agent.toml              # 来自 agents/precision-tuning-discovery.md（重命名）
├── agents/
│   ├── ascend-kernel-developer.md             # 保留作人类可读文档（不再被 Codex 直接吃）
│   ├── precision-tuning-discovery.md          # 保留作历史/standalone 路径
│   └── ... (其它不动)
└── ...
```

**设计决策**：
- **双持方案**：`.codex/agents/*.toml` 是 Codex 的 source-of-truth；`agents/*.md` 保留给人看、也保留给 `run_precision_tuning.sh` 的 `--agent` 继续引用。两者内容在本次修改前后**一次性同步**，此后除非显式需要，不强行绑定双向同步。
- **subagent 改名**：`precision-tuning-discovery` → `cann-debug-agent`。`cann-debug-agent.toml` 的 `name` 字段是 `cann-debug-agent`，这是 Codex 运行时识别/`spawn` 用的唯一标识。
...
Phase 4.4 (spawn): 写 handoff artifact，spawn cann-debug-agent
...
> **为什么不让 parent 尝试修 AST/Build 失败？** 因为 parent 原本靠 3 轮 Conductor 才能修；改线性后没有迭代配额，继续让 parent 单轮修复正确率不够，不如记失败由人看 trace。Phase 4.4 仅接数值类失败——这也正是 `cann-debug-agent` 的 prerequisite。

---

## 6. `cann-debug-agent` 的首次输入契约（**核心改动**）

### 6.1 背景

原 `precision-tuning-discovery` agent 设定 prerequisite：
> task 目录下已有 model.py、model_new_ascendc.py、kernel/ 目录，
> 且 evaluate_ascendc.sh 已报告 Numerical 失败（非 Build/Import 失败）。

在 standalone 启动（`run_precision_tuning.sh`）场景，这些条件是"历史遗留状态"——某次跑完 ascend-kernel-developer 留下的文件 + 人为判断"数值失败"。
...
```json
{
  ...
  "task_name": "<op_name 或 task 目录 basename，与 run_precision_tuning.sh 对齐>",
  "task_dir": "<绝对路径>",
  "op_name": "<算子名>",
  "npu": 0,
...
**字段用途**：
- `task_name` / `task_dir` / `op_name` / `npu` — 等价 `run_precision_tuning.sh` 的 `--task-dirs` + `--npus`。
...
在 `cann-debug-agent.toml` 的 `developer_instructions` 里，Step 2.1 的流程增加一个**前置 hook**：
...
| 路线 | 做法 | 取舍 |
|---|---|---|
| **A** 覆盖内置 `default` | `.codex/agents/default.toml` 的 `name = "default"`，描述里说 "本仓库专属：任何算子生成任务转交 `ascend-kernel-developer` / 任何精度调优任务转交 `cann-debug-agent`" | 用户敲 `codex "生成..."` 立即生效；但 `default` 名字被覆盖后其它非业务任务也会被这段描述影响 |
| **B** 文档化 `--agent` 调用 | 只写 `ascend-kernel-developer.toml` + `cann-debug-agent.toml`，在 `.codex/AGENTS.md` 告诉人类/自动化"用 `codex --agent ascend-kernel-developer "..."` 启动" | 干净、不污染 default；代价是每次多敲一个 flag |
...
- **新增**：`Phase 4.4 - Spawn cann-debug-agent` 小节，含 handoff 写入规范（引用 §6.2 schema）、spawn 口令的自然语言模板（见 §9）、返回值消费逻辑。
- **修改**："约束" 表里 "Phase 4 最大迭代 3 次" 一行删除，替换为 "Phase 4 线性执行 + 数值失败委派 cann-debug-agent subagent"。
...
Codex 原生规则：父 agent 在 `developer_instructions` 里**自然语言点名** `cann-debug-agent`，runtime 识别后自动 spawn。无额外 CLI 语法。
...
| 风险 | 说明 | 缓解 |
|---|---|---|
| `agents.max_depth` 默认 1 | parent 里 spawn 一次 OK；若 subagent 自己再 spawn 会被拦 | 保持默认；`cann-debug-agent` 不应再嵌套 spawn |
| handoff JSON 漂移 | 如 subagent 改 schema 导致 parent 写错 | 在 design.md 定稿后把 schema 冻结到 `.codex/agents/cann-debug-agent.toml` 的 `developer_instructions` 里 |
...
| Standalone 路径回归 | `run_precision_tuning.sh` 仍指向 `agents/precision-tuning-discovery.md` | 保留 .md 文件；一次性修正 .md 让它等价于新 TOML 的 behavior（或者不动，只在 TOML 里做 handoff 支持），下节决定 |

### 10.1 Standalone 路径兼容策略

两个方案：

- **方案 X**（最小改动）：只写 TOML，不动 `agents/precision-tuning-discovery.md`。handoff 逻辑只进 TOML。
  - 优点：向后兼容 standalone。
  - 缺点：两份文档漂移。
...
1. `.codex/agents/ascend-kernel-developer.toml` 与 `.codex/agents/cann-debug-agent.toml` 语法合法（`python -c "import tomllib; tomllib.load(...)"` 通过）。
2. `.codex/AGENTS.md` 描述项目默认任务 + 两个 agent。
3. `ascend-kernel-developer.toml` 的 Phase 4 内容严格线性，无 `while ac_iteration` 字样。
4. `cann-debug-agent.toml` 含 §6.3 的 Step 2.1 前置 hook 与 §6.4 的返回值规范。
5. Parent spawn 模板（§9）出现在 Phase 4.4。
6. `agents/precision-tuning-discovery.md` 按方案 Y 同步补 handoff hook；`agents/ascend-kernel-developer.md` 同步线性化（保证 md/TOML 一致）。
